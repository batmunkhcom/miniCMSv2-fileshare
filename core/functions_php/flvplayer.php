...
function mbmFlvPlayer($flvPlayerVars = array()){
	...
	//xmoovplayer
	$buf1 = '<object width="'.$flvPlayerVars['width'].'" height="'.$flvPlayerVars['height'].'">
			<param name="movie" value="'.INCLUDE_DOMAIN.'data/xmoovplayer/player.swf">
			<param name="flashvars" value="video='.$file_path.'&streamer=false&autoplay=true">
			<embed src="'.INCLUDE_DOMAIN.'data/xmoovplayer/player.swf?video='.$file_path.'&streamer=false&autoplay=true" width="'.$flvPlayerVars['width'].'" height="'.$flvPlayerVars['height'].'">
			</embed>
			</object>';
	return $buf;
}
...
function mbmFlvPlayerStream($flvPlayerVars = array()){
	$buf = '<embed src="'.INCLUDE_DOMAIN.'data/jwplayer/player.swf" 
		...
		>';
		//xmoovplayer
	$buf = '<object width="'.$flvPlayerVars['width'].'" height="'.$flvPlayerVars['height'].'">
			<param name="movie" value="'.INCLUDE_DOMAIN.'data/xmoovplayer/player.swf">
			<param name="flashvars" value="video='.$file_path.'&streamer=true">
			<embed src="'.INCLUDE_DOMAIN.'data/xmoovplayer/player.swf?video='.$file_path.'&streamer=true" width="'.$flvPlayerVars['width'].'" height="'.$flvPlayerVars['height'].'">
			</embed>
			</object>';
