...
mbmSetContentTitle("<?=$lang['photogallery']['image_files']?>");
mbmSetPageTitle('<?=$lang['photogallery']['image_files']?>');
show_sub('menu15');
...
if(isset($_GET['action'])){
	switch($_GET['action']){
		case 'st':
			$r_action = $DB->mbm_query("UPDATE ".PREFIX."gallery_files SET st='".$_GET['st']."' WHERE id='".$_GET['id']."'");
			if($r_action == 1){
		...
		break;
		case 'delete':
			$r_action = $DB->mbm_query("DELETE FROM ".PREFIX."gallery_files WHERE id='".$_GET['id']."'");
			if($r_action == 1){
		...
		break;
		case 'private':
			$r_action = $DB->mbm_query("UPDATE ".PREFIX."gallery_files SET private='".$_GET['private']."' WHERE id='".$_GET['id']."'");
			if($r_action == 1){
		...
		break;
		case 'user_upload':
			$r_action = $DB->mbm_query("UPDATE ".PREFIX."gallery_files SET user_upload='".$_GET['user_upload']."' WHERE id='".$_GET['id']."'");
			if($r_action == 1){
  ...
  $r_photogalleries = $DB->mbm_query($q_photogalleries);
	...
	echo '
	  <tr>
		...
		<td align="center">'.$DB->mbm_result($r_photogalleries,$i,"filesize").'</td>
		<td align="center"><a href="index.php?module=photogallery&cmd=photo_files&action=st&st='
		...
		<td align="center"><a href="index.php?module=photogallery&cmd=photo_files&action=private&private='
		...
		<td align="center">'.$DB->mbm_result($r_photogalleries,$i,"filetype").'</td>
