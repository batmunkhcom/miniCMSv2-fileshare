...
mbmSetContentTitle("<?= $lang['users']['users_profile']?>");
mbmSetPageTitle('<?= $lang['users']['users_profile']?>');
show_sub('menu4');
</script><?
...
}elseif($DB2->mbm_check_field('id',$_GET['user_id'],'users')==0){

	echo '<div id="errorMain">Please login first.</div>';

}else{

	$q_user_profile = "SELECT * FROM ".USER_DB_PREFIX."users WHERE id='".$_GET['user_id']."'";
