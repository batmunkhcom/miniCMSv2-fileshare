...
mbmSetContentTitle("<?=$lang["dic"]["add_word_to_encyc"]?>");
mbmSetPageTitle('<?=$lang["dic"]["add_word_to_encyc"]?>');
show_sub('menu13');
</script>
<?		
if($mBm!=1 || $_SESSION['lev']<$modules_permissions[$_GET['module']] || 
	...
	if(strlen($_POST['word'])<2){
		...
		$r_word_add = $DB->mbm_insert_row($data,"dic_encyc_words");
		if($r_word_add==1){
