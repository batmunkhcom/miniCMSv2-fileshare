...
if(isset($_POST['st'])){
	...
	switch($_GET['type']){
			...
			if($r_content_add==1 && $b!=2){
				...
				if($b!=2){
					...
					if($r_video_add==1){
						...
						//twitter ruu bichleg nemeh
						postToTwitter($_SESSION['user']['username'].' "'.$data['title'].'" видео нэмлээ. &raquo; '.shortURL(DOMAIN.DIR.'index.php?module=menu&cmd=content&menu_id='.(mbmReturnMenuId($data['menu_id'])).'&id='.$tmp_content_id));
		...
		case 'photo':
				...
				if($r_content_add==1){
					...
					//twitter ruu bichleg nemeh
					postToTwitter($_SESSION['user']['username'].' "'.$data['title'].'" нэртэй ('.count($_POST['img_title']).') зураг нэмлээ. &raquo; '.shortURL(DOMAIN.DIR.'index.php?module=menu&cmd=content&menu_id='.(mbmReturnMenuId($data['menu_id'])).'&id='.$tmp_content_id));
		...
		case 'normal':
			...
			if($r_content_add==1){
				...
				//twitter ruu bichleg nemeh
				postToTwitter($_SESSION['user']['username'].' "'.$data['title'].'" нэртэй мэдээ нэмлээ. &raquo; '.shortURL(DOMAIN.DIR.'index.php?module=menu&cmd=content&menu_id='.(mbmReturnMenuId($data['menu_id'])).'&id='.$DB->mbm_get_field($data['session_id'],'session_id','id','menu_contents')));
		...
		case 'youtube':
			...
			if($r_content_add==1 && $b!=2){
				...
				if($r_video_add==1){
					...
					//twitter ruu bichleg nemeh
					postToTwitter($_SESSION['user']['username'].' "'.$data['title'].'" нэртэй видео зураг нэмлээ. &raquo; '.shortURL(DOMAIN.DIR.'index.php?module=menu&cmd=content&menu_id='.(mbmReturnMenuId($data['menu_id'])).'&id='.$tmp_content_id));
	...
	if($b == 1){
		sendNewsletter(array(
						'subject'=>$data['title'],
						'from_email'=>'admin@mng.cc',
						'from_name'=>'MOD noos',
						'title'=>$data['title'],
						'content'=>$data['content_short']
