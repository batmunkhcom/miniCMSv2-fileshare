...
mbmSetContentTitle("<?=$lang["menu"]["content_permissions_edit"]?>");
mbmSetPageTitle('<?=$lang["menu"]["content_permissions_edit"]?>');
show_sub('menu2');
</script>
<?		
...
if(isset($_POST['updatePermissions'])){
	...
	if($b!=2){
		...
		$data['total_updated'] = $DB->mbm_get_field($_GET['id'],'id','total_updated','menu_permissions')+1;
		...
		if($DB->mbm_update_row($data,'menu_permissions',$_GET['id'])==1){
...
<table width="100%" border="0" cellspacing="2" cellpadding="3" class="tblContents">
  ...
  <tr>
    <td bgcolor="#f5f5f5"><?=$lang["main"]["username"]?>:<br><strong><?=$DB2->mbm_get_field($DB->mbm_get_field($_GET['id'],'id','user_id','menu_permissions'),'id','username','users')?></strong></td>
  ...
  <tr>
    <td bgcolor="#f5f5f5"><?=$lang['main']['status']?>:<br>
        <select name="st" id="st">
          <?=mbmShowStOptions($DB->mbm_get_field($_GET['id'],'id','st','menu_permissions'))?>
          <option value="2" <?
          if($DB->mbm_get_field($_GET['id'],'id','st','menu_permissions')==2){
		  	echo 'selected';
		  }
		  ?>><?=$lang['menu']['status_any']?></option>
        </select>    </td>
    <td bgcolor="#f5f5f5">&nbsp;</td>
  </tr>
  <tr>
  ...
  <tr>
    <td bgcolor="#f5f5f5"><?=$lang['main']['level']?>:<br>
        <select name="lev">
          <?= mbmIntegerOptions(0, 5,$DB->mbm_get_field($_GET['id'],'id','lev','menu_permissions')); ?>
      </select></td>
    <td bgcolor="#f5f5f5">&nbsp;</td>
  </tr>
  <tr>
  ...
  <tr>
    <td bgcolor="#f5f5f5"><?=$lang['main']['content_permissions']?>:<br>
      ...
      <table width="100%" border="0" cellspacing="2" cellpadding="0">
        ...
        <tr>
         ...
         if($DB->mbm_get_field($_GET['id'],'id','is_photo','menu_permissions')==1){
			echo ' checked="checked" ';
		}
		?> /></td>
          <td align="center" bgcolor="#eeeeee"><input type="checkbox" name="is_video" id="is_video" value="1" <?
         if($DB->mbm_get_field($_GET['id'],'id','is_video','menu_permissions')==1){
			echo ' checked="checked" ';
		}
		?> /></td>
          <td align="center" bgcolor="#eeeeee"><input type="checkbox" name="normal" id="normal" value="1" <?
         if($DB->mbm_get_field($_GET['id'],'id','normal','menu_permissions')==1){
			echo ' checked="checked" ';
		}
		?> /></td>
  ...
  <tr>
    <td bgcolor="#f5f5f5"><?=$lang["main"]["comment"]?>:<br>
      <textarea name="comment" cols="45" rows="3" id="comment" class="input"><?=$DB->mbm_get_field($_GET['id'],'id','comment','menu_permissions')?></textarea></td>
    <td bgcolor="#f5f5f5">&nbsp;</td>
  </tr>
  <tr>
    <td bgcolor="#f5f5f5">&nbsp;</td>
