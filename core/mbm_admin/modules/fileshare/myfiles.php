...
if($_SESSION['lev']==0){
	...
	$q_myfiles = "SELECT * FROM ".PREFIX."fileshare ORDER BY ".$order_by_ob." ".$asc_asc."";

	$r_myfiles = $DB->mbm_query($q_myfiles);

	

	$q_totalfilesize = "SELECT SUM(filesize),SUM(downloaded) FROM ".PREFIX."fileshare";
...
if($_POST['typeList']){
	...
	if(count($_POST['action_f'])>0){
		...
		$q_fileList = "SELECT `key`,`filesize`,`filename_orig` FROM ".PREFIX."fileshare WHERE ";
...
<div align="center" style="padding:5px; background-color:#f9f9f9; border:1px solid #e2e2e2;">
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1">
  ...
  <tr class="bold">
    ...
    <td width="20%" height="25" align="center" bgcolor="#EEEEEE"><a href="#" onclick="submitForm(1)">Холбоосууд харах</a></td>
    ...
    <td width="20%" align="center" bgcolor="#EEEEEE"><a href="#">Устгах</a></td>
...
<a href="#" onclick="toggleAll(2);return false;">Эсрэгээр</a>
  ...
  <table width="99%" border="0" cellspacing="1" cellpadding="2" style="border:1px solid #DDDDDD; margin-top:6px;">
    ...
    <tr class="bold">
      ...
      <td bgcolor="#e2e2e2"><a href="index.php?module=fileshare&amp;cmd=myfiles&ob=1&asc=<?=$asc_asc?>">Файлын нэр</a></td>

      <td width="40" align="center" bgcolor="#e2e2e2"><a href="index.php?module=fileshare&amp;cmd=myfiles&ob=6&asc=<?=$asc_asc?>">Хэрэглэгч</a></td>

      <td width="40" align="center" bgcolor="#e2e2e2"><a href="index.php?module=fileshare&amp;cmd=myfiles&ob=2&asc=<?=$asc_asc?>">Хоног</a></td>

      <td width="75" align="center" bgcolor="#e2e2e2"><a href="index.php?module=fileshare&amp;cmd=myfiles&ob=3&asc=<?=$asc_asc?>">Огноо</a></td>

      <td width="75" align="center" bgcolor="#e2e2e2"><a href="index.php?module=fileshare&amp;cmd=myfiles&ob=4&asc=<?=$asc_asc?>">Хэмжээ</a></td>

      <td width="75" align="center" bgcolor="#e2e2e2"><a href="index.php?module=fileshare&amp;cmd=myfiles&ob=5&asc=<?=$asc_asc?>">Хан./Тат.</a></td>
  ...
  ?>
    ...
    <tr>
      ...
      <td ondblclick="alert('<?=$DB->mbm_result($r_myfiles,$i,"filename_orig")?>')" title="<?=$DB->mbm_result($r_myfiles,$i,"abs_url")?>"><?
	...
	?></td>
	  ...
	  $u_name = $DB2->mbm_get_field($DB->mbm_result($r_myfiles,$i,"user_id"),'id','username','users');
      ...
      if($DB->mbm_result($r_myfiles,$i,"days_to_save")<7){

		  	echo '#FF0000" class="bold white';
