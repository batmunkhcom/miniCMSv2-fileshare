...
class DB{
	...
	function mbm_query($query){
		...
		//db name bichigdsen bol davhar bichigdehes sergiilj arilgana
		$query = str_replace($this->db['db_name'].'.','', $query);
		$query = str_replace($this->db['db_name'].' .','', $query);
		
		//urd n db_name zalgaj ugnu
		$query = str_replace($this->prefix,$this->db['db_name'].'.'.$this->prefix, $query);
