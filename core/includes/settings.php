...
global $DB, $DB2, $censored_words,$PHPMAILER, $BBCODE;
...
$q_settings = "SELECT * FROM ".$DB->prefix."settings ";
$r_settings = $DB->mbm_query($q_settings);


for($i=0;$i<$DB->mbm_num_rows($r_settings);$i++){
	define(strtoupper($DB->mbm_result($r_settings,$i,"name")),$DB->mbm_result($r_settings,$i,"value"));
	
}
if($_SESSION['lev']>0 && !isset($_SESSION['user']['username'])){
	$q_userinformation = "SELECT * FROM ".$DB2->prefix."users WHERE id='".$_SESSION['user_id']."'";
...
if(!is_array($_SESSION['country'])){
	...
	$country_info_xml = file_get_contents('http://ipinfodb.com/ip_query.php?ip='.getenv("REMOTE_ADDR").'');
	$ipINFO    = new XmlToArray(strtolower($country_info_xml));
	$country_info = $ipINFO->createArray();
	
	
	
	if($country_info['response']['countryname'] == ''){
		$GET_COUNTRY_NAME_ = mbmCountry();
		if(function_exists("mbmCountry")){
			$_SESSION['country']['name'] = $GET_COUNTRY_NAME_;
		}
		if($_SESSION['country']['name'] == ''){
			$_SESSION['country']['name'] = $GET_COUNTRY_NAME_;
		}
	}else{
		$_SESSION['country']['name'] = strtoupper($country_info['response']['countryname']);
	}
}
		...
		switch($_GET['action']){
			...
			case 'userLogin':
				if(isset($_POST['loginButton'])){
					...
					$q_userlogin = "SELECT * FROM ".USER_DB_PREFIX."users WHERE (username='".$_POST['username']."' OR email='".$_POST['username']."' )
					...
					header("Location: index.php?module=users&cmd=welcome");
						}
					}else{
			...
			case 'logout';
				...
				$data_logout['session_time'] = 0;
					header("Location: index.php?module=users&cmd=bye");
