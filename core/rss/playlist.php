...
if (!isset($_GET['user_id']) && !isset($_POST['user_id']) && !isset($_SESSION['user_id'])) {
    $userID = 0;
}
if ($_GET['private'] == 1) {
    $userID = $_SESSION['user_id'];
...
if (!isset($_GET['playlist_id']) && !isset($_POST['playlist_id'])) {
    $playlist_id = 0;
}
...
switch ($_GET['type']) {
    case 'xml': //odoogoor ajillahgui bna
        ...
        for ($i = 0; $i < $DB2->mbm_num_rows($r_playlist_videos); $i++) {
            ...
            $buf_rss .= '<item> '."\n".'
								<title>'.$DB2->mbm_result($r_playlist_videos, $i, "title").'</title>  '."\n".'
								<media:content url="'.$DB2->mbm_result($r_playlist_videos, $i, "video_url").'" type="video/x-flv" start="00:00" />  '."\n".'
								<media:thumbnail url="'.$DB2->mbm_result($r_playlist_videos, $i, "image_url").'" />  '."\n".'
								<description>'.$DB2->mbm_result($r_playlist_videos, $i, "comment").'</description>  '."\n".'
