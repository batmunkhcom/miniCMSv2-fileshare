...
function mbmDeleteFileFileshare($var = array(
	...
	if($DB->mbm_num_rows($r_checkfile)==1){
		$conn_id = @ftp_connect($ftp_config['server'],22);
		$ftp_login = @ftp_login($conn_id,$ftp_config['username'],$ftp_config['password']);

                $file_to_delete_abs_path = $DB->mbm_result($r_checkfile,0,'abs_url');
		$file_to_delete = $ftp_config['web_root'].str_replace(ABS_DIR,'',$file_to_delete_abs_path);
		
                        exec("rm -f ".$file_to_delete_abs_path);
...
function mbmFileshareFilelist($var =array(
	...
	$q = "SELECT * FROM ".DB_NAME.'.'.PREFIX."fileshare WHERE id!=0 ";
	if($var['is_private']!=2){
