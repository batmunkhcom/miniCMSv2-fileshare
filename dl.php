...
$q_file = "SELECT * FROM " . $DB->prefix . "fileshare WHERE `key`='" . addslashes($_GET['k']) . "'";
//        echo $q_file; exit();
...
//    header("Content-Disposition: attachment;filename=".$DB->mbm_result($r_file, 0, 'dl_url')." ");
//    header("Content-Transfer-Encoding: binary ");


//    header('Content-Description: File Transfer');
//    header('Content-Type: application/octet-stream');
//    header('Content-Disposition: attachment; filename="' . basename($DB->mbm_result($r_file, 0, 'dl_url')) . '"'); //<<< Note the " " surrounding the file name
//    header('Content-Transfer-Encoding: binary');
//    header('Connection: Keep-Alive');
//    header('Expires: 0');
//    header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
//    header('Pragma: public');
//    header('Content-Length: ' . $DB->mbm_result($r_file, 0, 'filesize'));

    header("Location: " . $DB->mbm_result($r_file, 0, 'dl_url'));
//		echo 1; die();
//            print_r($_SESSION);
