	...
	?></td>
            ...
            <tr>
              ...
              <td width="25%" align="right"><strong>
                <?=mbmPercent(($DB->mbm_result($r_daily,0,0)*$counter_by),($DB->mbm_result($r_total_hits,0,0)*$counter_by),0)?>
              </strong></td>
            </tr>
            <tr>
              <td><?=$lang["stats"]["max_hits"]?></td>
              <td align="right"><strong>
                <?=mbmPercent(ceil($DB->mbm_result($r_daily,0,1)*$counter_by),ceil($DB->mbm_result($r_daily,0,0)*$counter_by),1)?>
              </strong></td>
            </tr>
            <tr>
              <td><?=$lang["stats"]["min_hits"]?></td>
              <td align="right"><strong>
               <?=mbmPercent(ceil($DB->mbm_result($r_daily,0,2)*$counter_by),ceil($DB->mbm_result($r_daily,0,0)*$counter_by),2)?>
              </strong></td>
            </tr>
            <tr>
              <td><?=$lang["stats"]["average_hits"]?></td>
              <td align="right"><strong>
                <?=mbmPercent(ceil($DB->mbm_result($r_daily,0,3)*$counter_by),ceil($DB->mbm_result($r_daily,0,0)*$counter_by),3)?>
              </strong></td>
            </tr>
        </table>
		...
		for($i=0;$i<$DB->mbm_num_rows($r_monthly);$i++){
			echo '
				  <tr bgcolor="'.$row_bgcolor[$i%2].'">
					...
					.mbmPercent(ceil($DB->mbm_result($r_monthly,$i,"hits")*$counter_by),ceil($DB->mbm_result($r_daily,0,0)*$counter_by),4)
					.'</td>
				  </tr>';
		}
	  ?>
      ...
      <tr>
        <td><table width="100%" border="1" cellspacing="0" cellpadding="3" style="border-collapse:collapse;">
            ...
            <tr>
              <td><?=$lang["stats"]["total_hits"]?></td>
              <td align="right"><strong><?=number_format($DB->mbm_result($r_total_hits,0,0)*$counter_by,0)?></strong></td>
            </tr>
            <tr>
              <td><?=$lang["stats"]["max_hits"]?></td>
              <td align="right"><strong>
                <?=number_format($DB->mbm_result($r_total_hits,0,1)*$counter_by,0)?>
              </strong></td>
            </tr>
            <tr>
              <td><?=$lang["stats"]["average_hits"]?></td>
              <td align="right"><strong>
                <?=number_format($DB->mbm_result($r_total_hits,0,2)*$counter_by,0)?>
              </strong></td>
            </tr>
            <tr>
              <td><?=$lang["stats"]["min_hits"]?></td>
              <td align="right"><strong>
                <?=number_format($DB->mbm_result($r_total_hits,0,3)*$counter_by,0)?>
              </strong></td>
            </tr>
            <tr>
      ...
      <tr>
		...
		for($i=0;$i<$DB->mbm_num_rows($r_monthly2);$i++){
			...
			echo mbmPercent(ceil($DB->mbm_result($r_monthly2,$i,2)*$counter_by),ceil($DB->mbm_result($r_total_hits,0,0)*$counter_by),0);
			echo '</td></tr>';
		}
	 ?>
        </table>
      ...
      <tr>
		...
		for($i=0;$i<$DB->mbm_num_rows($r_countries2);$i++){
			...
			echo '</td><td width="25%" align="right" class="bold">';
			echo mbmPercent(ceil($DB->mbm_result($r_countries2,$i,"hits")*$counter_by),ceil($DB->mbm_result($r_countries,0,1)*$counter_by),1);
			echo '</td></tr>';
		}
	?>
        </table></td>
      ...
      <tr>
		...
		for($i=0;$i<$DB->mbm_num_rows($r_referers2);$i++){
			...
			echo '</td><td width="25%" align="right" class="bold">';
			echo mbmPercent(ceil($DB->mbm_result($r_referers2,$i,"hits")*$counter_by),ceil($DB->mbm_result($r_referers,0,1)*$counter_by),2);
			echo '</td></tr>';
		}
	 ?>
        </table></td>
